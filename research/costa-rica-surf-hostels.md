---
status: provisional
sources:
  - external-sources/costa-rica-surf-hostels.md
  - external-sources/surf-independent-lessons-rentals.md
  - external-sources/surf-conditions-august.md
  - external-sources/cr-don-jons-lost-boyz.md
created: 2026-06-02
author: Miles Kaming-Thanassi
tags: [ research, provisional, surfing ]
---

## Question

Find **surf hostels in Costa Rica, preferably locally/Tico-owned, with lesson + board-rental options** — the budget, independent end of the spectrum (not the big international camps). Companion to the [Costa Rica proposal](../trips/surf-costa-rica-nosara.md) and [camp comparison](./surf-camp-comparison.md).

## Sources cited

- [CR surf hostels & local schools](../external-sources/costa-rica-surf-hostels.md)
- [À-la-carte lesson/rental pricing](../external-sources/surf-independent-lessons-rentals.md)
- [August conditions](../external-sources/surf-conditions-august.md)

## Read this first — the honest bit on "locally owned"

> [!IMPORTANT]
> **Verifying Tico ownership of a hostel from the internet is hard** — most listings never state owner nationality, and the biggest "surf hostels" (e.g. **Selina** Nosara/Jacó) are **international chains**. The most reliably local approach is a **two-part combo: a small independent hostel + a confirmed Tico-owned surf school** for your lesson(s) and board rental. That also fits your à-la-carte preference perfectly. Confirmed locally-owned *schools* below: **Tico's (Sámara)**, **Avellanas (Mauricio)**, **Young Visions (Montezuma)**.

## Hostels by town (with lessons/rental)

| Hostel | Town | Dorm/night | Lessons + rental | Local-owned confidence | Vibe |
| --- | --- | --- | --- | --- | --- |
| **Zeneida's Surf Garden** | Santa Teresa | ~$13 | Packages + board rental | 🟡 *name suggests local; unconfirmed* | Beachfront, social ([src](../external-sources/costa-rica-surf-hostels.md)) |
| **El Gato Rojas Surf Hostel** | Santa Teresa | budget | Cheap board rental | 🟡 *local-sounding; unconfirmed* | Budget |
| **Mal País Surf Camp & Hotel** | Mal País | dorm→room | Rentals + lessons | 🟡 *long-standing* | Gardens, mixed |
| **Hostel Nosara** | Nosara (Guiones) | budget | Connected board rental | 🟡 *unconfirmed* | Jungle, social, 100 m to beach |
| **Casa Pura Vida** | Tamarindo | $15–20 | (use Iguana Surf nearby) | 🟢 *independent, family-run* | "Feels like family" |

## Confirmed Tico-owned surf schools (pair with a cheap hostel)

| School | Town | Lessons | Rentals | Why it's the local pick |
| --- | --- | --- | --- | --- |
| **Tico's Surf School** | Playa Sámara | $60 group / $80 private | soft/hard, SUP, boogie | Est. 2008; **ISA-cert instructor-lifeguards**; calmest beginner bay ([src](../external-sources/costa-rica-surf-hostels.md)) |
| **Nosara Tico Surf School** | Nosara (Guiones) | private + group | yes | **"100% Tico" bilingual instructors, ISA & RLSS certified**; conditions-reading focus ([src](../external-sources/costa-rica-surf-hostels.md)) |
| **Avellanas Surf School** | Playa Avellanas | beginner→advanced | yes | **Owned by Mauricio**; 20+ years |
| **Young Visions** | Montezuma | all levels | yes | **"Tico-owned and run,"** #1-rated locally |

## Recommendation for your group

> [!TIP]
> - **Best overall (local + beginner-friendly) → Playa Sámara: cheap local hostel + [Tico's Surf School](../external-sources/costa-rica-surf-hostels.md).** Sámara's calm, reef-protected bay is the gentlest August option, lessons are $60 group, and Tico's is the clearest locally-owned operator with full rentals. Quieter/less party than Santa Teresa.
> - **Best beachfront budget → Zeneida's Surf Garden (Santa Teresa), ~$13/night**, packages + rental on the sand — then add à-la-carte lessons. Livelier town, punchier waves.
> - **Best "feels-like-family" → Casa Pura Vida (Tamarindo)** + **Iguana Surf** (beachfront rentals/lessons since 2001) or **Avellanas Surf School** (Mauricio). Tamarindo is forgiving in August.
> - **Skip if local ownership matters:** Selina (chain).

This combo (independent hostel + Tico school) typically runs **~$13–25/night lodging + $60 lesson + $15–25/day board** — the cheapest and most local way to do the [Costa Rica week](../trips/surf-costa-rica-nosara.md). See also [August conditions](./surf-conditions-august.md): Sámara/Tamarindo gentlest, Santa Teresa punchiest.

## Santa Teresa head-to-head: Don Jon's vs Lost Boyz

Two popular Santa Teresa surf stays — both steps from the beach, both all-levels — but aimed at different travelers ([src](../external-sources/compare-don-jons-lost-boyz.md)).

| | **Don Jon's Surf & Yoga Lodge** | **Lost Boyz Hostel & Surf Camp** |
| --- | --- | --- |
| **Vibe** | Chill, yoga, "relax to the max"; couples/families + backpackers | Social/party backpacker community; pool parties, bar |
| **Location** | ~100 m to beach; ST beach break "a few steps away" | Across from beach; 3-min walk, 5 min to best breaks |
| **Rooms** | Apartments (AC) · private teak cabins · AC hostel beds; **~$60–90/nt** private | **Dorms from ~$22/nt** + private rooms; pool, treehouse |
| **Surf model** | **Bundled camp** (5+ days: lodging + daily breakfast + daily lesson + board) | **À-la-carte packs** ($250–650/wk: 2–5 lessons + 7-day board rental); lodging separate |
| **Lessons** | Private 1:1 ~$60; local certified instructors | Included in packs; transport + rashguard + sunblock |
| **Extras** | Free yoga, on-site restaurant | Pool, bar, ATV, Isla Tortuga, bioluminescence, photo/video |
| **Best for** | Couples/families; calm + comfort + simplicity | Young, social, budget; flexibility + nightlife |

> [!TIP]
> For a 2–4-person advancing-beginner group: **Lost Boyz** if you want social/lively + budget + flexible lessons (and don't mind a party hostel); **Don Jon's** if you want a calmer, more comfortable base with bundled daily coaching + yoga. Both sit on Santa Teresa's **punchier beach break**, so book coached **morning** sessions either way ([August conditions](./surf-conditions-august.md)). Neither is confirmed Tico-owned — if local ownership matters, see the confirmed-local schools above.

## Open questions

- Confirm ownership directly when booking (message the hostel — many family places answer warmly and will tell you).
- Sámara vs Santa Teresa vs Tamarindo as base — calm/local vs lively vs forgiving-classic.
- Verify August dorm availability + exact rental prices (and whether the hostel bundles a lesson discount).
- ⚠️ **Nosara green-season access:** poorly maintained roads + frequent **bridge washouts** in the rainy season — build in transfer buffer (or favor Sámara/Tamarindo, which are easier to reach) for an August trip.
- Confirm Tico's Surf School rental day-rates (lesson prices known; rental not listed online).
